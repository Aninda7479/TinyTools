...
async function runWasmSplit(_input: string, _outputDir: string, _rows: number, _cols: number): Promise<ToolResult> {
  return { success: false, output_path: null, message: "Split Image is not yet supported in the web browser" };
}

async function runWasmStitch(_paths: string[], _output: string, _direction: string): Promise<ToolResult> {
  return { success: false, output_path: null, message: "Stitch Images is not yet supported in the web browser" };
}
